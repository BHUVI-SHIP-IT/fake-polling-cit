...
/**
 * Complete Excel to CSV Converter with Student Names
 * This script converts the multi-sheet competitive IDs file into one complete CSV
 * that includes ALL students with their names and competitive programming IDs.
 */

function completeConversionWithNames() {
  try {
    console.log('🚀 Starting Complete Excel to CSV conversion with names...\n');
    
    // Configuration
    const STUDENT_INFO_FILE = './student-info.xlsx';
    const COMPETITIVE_IDS_FILE = './competitive-ids.xlsx';
    const OUTPUT_CSV = './students-complete-with-names-and-competitive-ids.csv';
    ...
    for (let i = 0; i < studentData.length; i++) {
      ...
      const studentName = student['__EMPTY_2'] || student['Name'] || 'Unknown Student';
    ...
    // Create CSV content
    console.log('📝 Creating complete CSV file with names...');
    
    const csvHeaders = 'rollNumber,collegeEmail,name,personalEmail,year,section,department,leetcodeId,leetcodeContestId,codechefId,codeforcesId,otherIds';
    const csvRows = convertedStudents.map(student => 
      `${student.rollNumber},${student.collegeEmail},${student.name},${student.personalEmail},${student.year},${student.section},${student.department},${student.leetcodeId || ''},${student.leetcodeContestId || ''},${student.codechefId || ''},${student.codeforcesId || ''},${student.otherIds}`
    );
    ...
    console.log(`🎉 Complete conversion with names finished successfully!`);
    ...
    withCompetitiveIds.slice(0, 5).forEach((student, index) => {
      console.log(`   ${index + 1}. ${student.rollNumber} | ${student.name} | ${student.collegeEmail} | Section: ${student.section}`);
    ...
    console.log('\n🚀 Next Steps:');
    console.log('   1. Run database migration: cd server && npm run prisma:migrate');
    console.log('   2. Review the generated CSV file');
    console.log('   3. Upload to the system using Bulk Upload feature');
    console.log('   4. All students will have names and competitive programming IDs!');
...
// Run the complete conversion with names
completeConversionWithNames(); 